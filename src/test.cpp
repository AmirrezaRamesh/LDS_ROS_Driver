...
void LFCDLaser::poll(sensor_msgs::msg::LaserScan::SharedPtr scan)
{
  ...
  while (!shutting_down_ && !got_scan)
  {
    ...
    if (start_count == 0 && raw_bytes[start_count] == 0xFA)
    {
      start_count = 1;
    }
    else if (start_count == 1)
    {
      if (raw_bytes[start_count] == 0xA0)
      {
        ...
        scan->angle_min = M_PI / 2;             // 90 degrees
        scan->angle_max = 3.0 * M_PI / 2;       // 270 degrees
        scan->range_min = 0.12;
        scan->range_max = 3.5;

        std::vector<float> full_ranges(360, std::numeric_limits<float>::quiet_NaN());
        std::vector<float> full_intensities(360, 0.0);
        ...
        for (uint16_t i = 0; i < raw_bytes.size(); i = i + 42)
        {
          if (raw_bytes[i] == 0xFA && raw_bytes[i + 1] == (0xA0 + i / 42))
          {
            ...
            for (uint16_t j = i + 4; j < i + 40; j = j + 6)
            {
              ...
              full_ranges[359 - index] = range / 1000.0;
              full_intensities[359 - index] = intensity;
            }
          }
        }

        scan->ranges.assign(full_ranges.begin() + 90, full_ranges.begin() + 271);
        scan->intensities.assign(full_intensities.begin() + 90, full_intensities.begin() + 271);

        rpms = motor_speed / good_sets / 10;
        scan->time_increment = 1.0f / (rpms * 6);
        scan->scan_time = scan->time_increment * 180;  
      }
      else
      {
        start_count = 0;
